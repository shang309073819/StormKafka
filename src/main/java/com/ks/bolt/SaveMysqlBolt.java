...
public class SaveMysqlBolt extends BaseBasicBolt {
	...
	private static Map<String, String> memberMap = null; // sendpay,
															// counterMember
	...
	public void prepare(Map stormConf, TopologyContext context) {
		...
		jedisClient = RedisUtils.getInstance();
	...
	private void saveMap(String sendpay, boolean isHasMem, String totalprice,
			String youhui) {
		if (isOpen) {
			if (checkMap()) {
				...
				if (value != null) {
					...
					double etp = Double.valueOf(vals[2])
							+ (Double.valueOf(totalprice) - Double
									.valueOf(youhui));
				...
				System.out.println("sendpay = " + sendpay + "  value = "
						+ value);
				memberMap.put(sendpay, value);
			}
		} else {
			String value = sendpay + "_" + 1 + "," + totalprice + ","
					+ (Double.valueOf(totalprice) - Double.valueOf(youhui))
					+ "," + (isHasMem ? 0 : 1);
			cacheList.add(value);
		}
	}
	...
	private void saveMysql(Map<String, String> tmpMap) {
		...
		InterProcessMutex lock = new InterProcessMutex(LockCuratorSrc.getCF(),
				Constant.LOCKS_ORDER);
		try {
			while (lock.acquire(10, TimeUnit.MINUTES)) {
				...
				for (Map.Entry<String, String> entry : tmpMap.entrySet()) {
					...
					// select ...
					String sql = "select id,order_nums,p_total_price,y_total_price,order_members from total_order where sendpay='"
							+ key + "'";
					...
					if (id == 0) {
						sBuffer.append(
								"insert into total_order(order_nums,p_total_price,y_total_price,order_members,sendpay) values(")
								.append(order_nums + "," + p_total_price + ","
										+ y_total_price + "," + order_member
										+ ",'" + key + "')");
					} else {// update
						sBuffer.append(
								"update total_order set order_nums="
										+ order_nums)
